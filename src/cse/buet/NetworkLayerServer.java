...
//        DVR(1); //Update routing table using distance vector routing until convergence
        simpleDVR(1);
        System.out.println("S dvr finished");
    ...
    public static synchronized void DVR(int startingRouterId) {
        ...
        */
        ...
        do{
            updated = false;
            Set<Integer> orderedIDSet = routerMap.keySet();
            List<Integer> orderedIDs = new LinkedList<>();
            for (Integer id :
                    orderedIDSet) {
                if (id == startingRouterId) orderedIDs.add(0, id);
                else orderedIDs.add(id);
            }

            for (int id : orderedIDs) {
                Router r = routerMap.get(id);
//            List<RoutingTableEntry> T = r.getRoutingTable();
                for (int rid : r.getNeighborRouterIDs()) {
                    Router t = routerMap.get(rid);
                    if (t.getState()) {
                        updated |= t.sfupdateRoutingTable(r);
                    }
                }
            }
        }while (updated);
    ...
    public static synchronized void simpleDVR(int startingRouterId) {
        ...
        do{
            ...
            for (int id : orderedIDs) {
                Router r = routerMap.get(id);
                if(r == null ) System.out.println("id not here"+id);
                else
                for (int rid : r.getNeighborRouterIDs()) {
                    Router t = routerMap.get(rid);
                    if (t.getState()) {
                        updated |= t.updateRoutingTable(r);
                    }
                }
            }
        }while (updated);
